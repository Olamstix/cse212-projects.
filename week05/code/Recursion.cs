...
public static class Recursion
{
    ...
    {
        // Base case: n <= 0 contributes nothing
        if (n <= 0)
            return 0;
    ...
    {
        // Base case: we've built a permutation of the desired length
        if (word.Length == size)
        {
            results.Add(word);
            return;
        ...
        // Try adding each letter that hasn't been used yet in this permutation
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        // Recursive case: sum of ways from the last 1, 2, or 3 steps
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        // Base case: no wildcards left — pattern is a complete binary string
        if (wildcardIndex == -1)
        {
            results.Add(pattern);
            return;
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        // Backtrack: remove current position so other paths can explore freely
